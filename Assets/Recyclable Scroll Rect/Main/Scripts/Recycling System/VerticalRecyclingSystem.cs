...
public class VerticalRecyclingSystem : RecyclingSystem
{
    ...
    public override ICell GetCell(int cellIndex)
    {
        if(cellIndex < DataSource.GetItemCount())
        {
            // Check if cell is visible
            if(cellIndex < topMostCellIndex && cellIndex > bottomMostCellIndex)
            {
                return null;
            }
            else
            {
                ...
                if (cellIndex >= _cachedCells.Count)
                    cellIndex = cellIndex % _cachedCells.Count;

                return _cachedCells[cellIndex];
            }
